...
def PARAM_dict(dimension):
    return {
        "Numb_Dimensions": {"type": int, "Rec": dimension, "bounds": [1, 3]},
...
def SET_PARAMETERS():
    ...
    """
    Inp = input("Recommended or Custom settings?(r/c) ")
    Input_parameters = []
    ...
    # elec:0,muon:2,quarks: 6-11,photons:13
    match Inp.split():
        case ["r" | "recommended" | "rec", ("1" | "2" | "3") as dim_numb]:
            dim_numb = int(dim_numb)
            Parameter_dict = PARAM_dict(dim_numb)
        ...
        case _:
            ...
            for p_numb, param in enumerate(Params):
                ...
                while True:
                    if isinstance(User_inp, str) and User_inp.lower() in [
                        "r",
                        "recommended",
                        "rec",
                    ]:
