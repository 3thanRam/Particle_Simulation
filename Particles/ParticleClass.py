...
def BOUNDS(xi,xf,V,t,id,p,Mass):
    ...
    for d in range(DIM_Numb):
        ...
        if Global_variables.BoundSup_V[d]!=V[d]:
            SOL_sup=ROUND((b-Global_variables.BoundSup_b[d])/(Global_variables.BoundSup_V[d]-V[d]))
            CONDsup=t-dt<=SOL_sup<=t
        if Global_variables.BoundInf_V[d]!=V[d]:
            SOL_inf=ROUND((b-Global_variables.BoundInf_b[d])/(Global_variables.BoundInf_V[d]-V[d]))
        ...
        if CONDinf:
            if BOUNDARY_COND==0:
                ...
                if -V[d]+(Global_variables.BoundInf_V[d])<Global_variables.BoundInf_V[d]:
                    Vchgsign[d]=np.sign(Global_variables.BoundInf_V[d])*abs(V[d])+(Global_variables.BoundInf_V[d])
                else:
            ...
            if BOUNDARY_COND==0:
                ...
                if -V[d]+(Global_variables.BoundSup_V[d])>Global_variables.BoundSup_V[d]:
                    Vchgsign[d]=np.sign(Global_variables.BoundSup_V[d])*abs(V[d])+(Global_variables.BoundSup_V[d])
                else:
    ...
    if NZ>0:
        ...
        for nz in range(NZ):
            ...
            if nz==0:
                Xinter[nz][0]=np.where((Xini+V*dt>Linf_tv_d)&(Xini+V*dt<L_tv_d),Xini+V*dt,Xini)
                Xinter[nz][1]=np.where((Xini+V*dt>Linf_tv_d)&(Xini+V*dt<L_tv_d),Xini+V*dt,Xini)
                Vinter[d_arg]=Vchgsign[d_arg]
            else:
                val0=Xinter[nz-1][0]+Vinter*(t_val-(t-dt))#-t_list[-2])
                Xinter[nz][0]=np.where((val0>Linf_tv_d)&(val0<L_tv_d),val0,Xinter[nz-1][0])
                val1=Xinter[nz-1][1]+Vinter*(t_val-(t-dt))
                Xinter[nz][1]= np.where((val1>Linf_tv_d)&(val1<L_tv_d),val1,Xinter[nz-1][1])
...
class Particle:
    ...
    def __post_init__(self):
        ...
        if not self.ExtraParams:
            ...
            X=GEN_X()
            self.X=X
            Global_variables.TRACKING[typeIndex][partORanti][id].append([0.,X])
            if self.M!=0:
                V=GEN_V()
                ...
                self.Energy=((self.M*C_speed**2)**2+np.linalg.norm((self.P*C_speed)**2))**0.5
            else:
                v=GEN_V()
            ...
            PosParam,VParam,TvalueParam,Energyval=self.ExtraParams[1:]#PosParam,VParam,TStepsNumbParam=ExtraParams[1:]
            X=PosParam
            self.X=X
            self.V=VParam
    ...
    def DO(self,t):
        ...
        self.V=np.where((xf>Global_variables.L),-np.abs(Vt),np.where((xf<Global_variables.Linf),np.abs(Vt),Vt))
