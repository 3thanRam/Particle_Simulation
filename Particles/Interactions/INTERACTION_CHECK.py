...
def MINIMISE(difA, difB, t, Sa, Sb):
    ...
    Tsol = Tsol[(0.5*(Sa*Tsol+Sb) > L_FCT[1](Tsol)) & (0.5*(Sa*Tsol+Sb) < L_FCT[0](Tsol))& (Tsol < t) & (Tsol > t - dt)]
    ...
    if DIM_Numb == 1:# If there are no possible meeting times and there is only one dimension, return -1.
        return -1
...
def INTERCHECK_1D(a1,b1,p1,a2,b2,p2,t,z1,z2,Tstart,Tend):
    ...
    if difA!=0:
        ...
        if Tstart<=ti<=Tend and t-dt<=ti<=t and ti>0 and Global_variables.Linf<xo<Global_variables.L:
