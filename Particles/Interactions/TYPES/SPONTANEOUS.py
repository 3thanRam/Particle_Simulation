...
def STRONG_FORCE_GROUP(SystemList,TotnumbAllpart):
    ...
    Xarray=np.array([[(s.X[d],s.parity[0],s.parity[1],s.ID) for s in SystemList] for d in range(DIM_Numb)], dtype=dtype)  
    ...
    def QUARK_REGROUP(Numb_Meson,Numb_3Baryon,Quark_Group):
        ...
        if Numb_Meson!=0:
            ...
            if Numb_3Baryon!=0:
                NEW_Quark_Group.extend(Quark_Group[2*Numb_Meson:2*Numb_Meson+3*Numb_3Baryon])
...
def SpontaneousEvents(t):
    ...
    #ELM######
    ##########
    PARTICLE_SPONT_NAMES=['up_Quark','down_Quark','strange_Quark','charm_Quark','bottom_Quark','top_Quark']
    ...
    EcreationMinList=[2*PARTICLE_DICT[partName]['mass']*C_speed**2 for partName in PARTICLE_SPONT_NAMES]
    
    RemoveTypeInd=PARTICLE_DICT['photon']['index']
    Energy_List=np.array([photon.Energy for photon in Global_variables.SYSTEM[RemoveTypeInd][0]])
    ...
    for Eminindex,Ecreatmini in enumerate(EcreationMinList):
        P_list=np.where(Energy_List_CUT>Ecreatmini,np.exp(Energy_List_CUT/(Ecreatmini*Emax))-1,0)
    ...
    for nameind,killind in GEN_INFO:
        ...
        Posdeviation=min(np.min(np.abs(Global_variables.L-PosCenter)),np.min(np.abs(Global_variables.Linf-PosCenter)))/(10*max(abs(Global_variables.L-Global_variables.Linf)))
        ...
        VParam=(photDirection/(C_speed*NEWMASS))*((Energyval**2-NEWMASS**2*C_speed**4)/(C_speed**2*NEWMASS))**0.5 #V=p/m=pc/mc=direction*sqrt(Etot**2-Emass**2)/mc
        if DIM_Numb==3:
            theta=45*np.pi/180
            VParam1=np.round(rotate_quat(angle_axis_quat(theta, photDirection), VParam),decimals=10)
            VParam2=np.round(rotate_quat(angle_axis_quat(-theta, photDirection), VParam),decimals=10)
        elif DIM_Numb==2:
            theta=45*np.pi/180
        ...
        for i in range(2):
            ...
            Global_variables.SYSTEM[Typeindex][i].append(Particle(name=NewpartName,parity=(i,Typeindex),ID=idi,ExtraParams=CREATEPARAMS[i],Colour_Charge=COLOUR[i]))
    ...
    for Eminindex,Ecreatei in enumerate(EcreateLIST):
        ...
        P_list=np.where(SPRING_ENERGY_CUT>Ecreatei,np.exp(SPRING_ENERGY_CUT/(Ecreatei*Emax))-1,0)
        ...
        if np.trim_zeros(P_list.flatten()).size>0:
            ...
            for NonzeroNumb in range(len(Nonzero_inds[0])):
                quark1_ind,quark2_ind=Nonzero_inds[0][NonzeroNumb],Nonzero_inds[1][NonzeroNumb]
    ...
    for GENnumb,(nameind,ParentSystindex1,ParentSystindex2) in enumerate(GEN_INFO):
        ...
        parity1,id1,X1=SystemList[ParentSystindex1].parity,SystemList[ParentSystindex1].ID,np.array(SystemList[ParentSystindex1].X)
        PartOrAnti1,ind1=parity1

        parity2,id2,X2=SystemList[ParentSystindex2].parity,SystemList[ParentSystindex2].ID,np.array(SystemList[ParentSystindex2].X)
        ...
        VParam=(ParentDirection/(C_speed*NEWMASS))*((Energyval**2-NEWMASS**2*C_speed**4)/(C_speed**2*NEWMASS))**0.5 
        VParam1,VParam2=VParam,-VParam
        CREATEparam1='Spont_Create',PosParam1,VParam1,t,Energyval
        CREATEparam2='Spont_Create',PosParam2,VParam2,t,Energyval
        CREATEPARAMS=[CREATEparam1,CREATEparam2]
        ...
        for i in range(2):
            ...
            idi=Global_variables.MaxIDperPtype[Typeindex][i]
            Global_variables.SYSTEM[Typeindex][i].append(Particle(name=NewpartName,parity=(i,Typeindex),ID=idi,ExtraParams=CREATEPARAMS[i],Colour_Charge=COLOUR[i]))
            Global_variables.TRACKING[Typeindex][i][idi].insert(0,[t,CREATEPARAMS[i][1]])
