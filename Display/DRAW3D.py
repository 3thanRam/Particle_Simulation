...
def TRAJ_3D(dt,BOUNDARY_COND,Density,ALL_TIME,TRACKING,COLPTS,L_HIST,File_path_name):
    """
    ...
    Part_AntiPart_Data=[[[[[],[],[]] for tnumb in range(len(ALL_TIME))],[[[],[],[]] for tnumb in range(len(ALL_TIME))]] for p in range(len(ACTIVE_PARTS))]
        ...
        for ActPart_index,(antiorpartindex,particleindex) in enumerate(ACTIVE_PARTS):
                for partdata in TRACKING[particleindex][antiorpartindex]:
                    if not isinstance(partdata[0][time_ind],str):
                        for d in range(3):
                            Part_AntiPart_Data[ActPart_index][antiorpartindex][time_ind][d].append(partdata[d][time_ind])
    ...
    if COLPTS.ndim>1:
        ...
        for timef in Time:
            ...
            freqabso=OnlyAbso[(OnlyAbso[:,0]<=timef+ExtTparam_max)&(OnlyAbso[:,0]>=max(timef-ExtTparam_min,0))].shape[0]
            freqCreate=OnlyCreat[(OnlyCreat[:,0]<=timef+ExtTparam_max)&(OnlyCreat[:,0]>=max(timef-ExtTparam_min,0))].shape[0]
    ...
    def GEN_CUBOID(L_up,L_down):
        ...
        R0 = [np.linspace(-radius+L_down[i],L_up[i]+radius,10**3) for i in range(3)]
    ...
    for ActPart_index,(antiorpartindex,particleindex) in enumerate(ACTIVE_PARTS):
        DENSITY[ActPart_index][antiorpartindex]=[len(Part_AntiPart_Data[ActPart_index][antiorpartindex][tvalind][0]) for tvalind in range(len(Time)) ]
    ...
    PARTICLE_PENS=[]
    for ActPart_index,(antiorpartindex,particleindex) in enumerate(ACTIVE_PARTS):
        PARTICLE_PENS.append(mlab.points3d([0.5], [0.5], [0.5],color=colors.to_rgb(PARTICLE_DICT[PARTICLE_NAMES[particleindex]]['color']),mode=PART_OR_ANTI_MARKER[antiorpartindex],scale_mode='scalar',scale_factor=PartSize).mlab_source )



    def Disp(tparam,tparam_index):
        
        Npartparam=['\n   '+'Anti'*antiorpartindex+PARTICLE_NAMES[particleindex]+':'+str(DENSITY[ActPart_index][antiorpartindex][tparam_index]) for ActPart_index,(antiorpartindex,particleindex) in enumerate(ACTIVE_PARTS)]
    ...
    ABSO=[mlab.mesh(0*x_v,0*y_v, 0*z_v,scalars=s,color=(0,1,0)).mlab_source for i in range(freqmaxAbs)]
    CREAT=[mlab.mesh(0*x_v,0*y_v, 0*z_v,scalars=s,color=(1,0,1)).mlab_source for i in range(freqmaxCre)]
    BOUND_DRAW0=mlab.plot3d(Xlist,Ylist,Zlist,line_width=0.01,color=(0,0,0),tube_radius=0.1).mlab_source
    ...
    def update_scene(i):
        ...
        mlab.view(azimuth=theta,elevation=theta/2,distance=3.5*max(Lsup-Lmin),focalpoint=((Lsup[0]+Lmin[0])/2,(Lsup[1]+Lmin[1])/2,(Lsup[2]+Lmin[2])/2)) #,roll=theta
        ...
        for abempty in range(len(ABSO)-ab):
            if (ABSO[-1-abempty].x!=0*x_v).all():
                ABSO[-1-abempty].reset(x=0*x_v,y=0*y_v,z=0*z_v,color=(0,1,0))
        for Creatempty in range(len(CREAT)-cr):
            if (CREAT[-1-Creatempty].x!=0*x_v).all():
                CREAT[-1-Creatempty].reset(x=0*x_v,y=0*y_v,z=0*z_v,color=(0,1,0))
        #Plot Particles

        #for ActPart_index,(antiorpartindex,particleindex) in enumerate(ACTIVE_PARTS):
        for pen_ind,pen in enumerate(PARTICLE_PENS): #Draw each particle and interaction point
