...
def COMPLETE_DATAPTS(DATApts,TIMEpts,DIM_Numb):
    ...
    if DIM_Numb==2:
        ...
        def EMPTY_DOUBLE_LIST(DIM_Numb,dataN,DATA):
            Particle_list=[[[] for di in range(DIM_Numb)] for n in range(len(DATA[dataN][0]))]
            ANTIPart_list=[[[] for di in range(DIM_Numb)] for n in range(len(DATA[dataN][1]))]
            ...
            for dataindex,particledata in enumerate(partdata):
                ...
                for timeindex,timevalue in enumerate(TIMEpts):
                    if timevalue==Tend:
                        ...
                        Newparticledata[timeindex+1:]=[DEADpart]*(len(TIMEpts)-timeindex-1)

                        if DIM_Numb==3:
                            POS=particledata[-1][1]
                            for dim in range(DIM_Numb):
                                PART[p][partORanti][dataindex][dim].append(POS[dim])
                                PART[p][partORanti][dataindex][dim].extend(['D']*(len(TIMEpts)-timeindex-1))   
                        break
