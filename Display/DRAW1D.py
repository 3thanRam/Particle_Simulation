...
def TRAJ_1D(ax,TRACKING,Trange,COLPTS,L_HIST):
    ...
    for p1 in range(len(TRACKING)):
        for p0 in range(len(TRACKING[p1])):
            if len(TRACKING[p1][p0])!=0:
                lines.append(Line2D([], [], color=PARTICLE_DICT[PARTICLE_NAMES[p1]]['color'], linewidth=3, marker=PART_Style,linestyle=PART_LStyle[p0]))
                    ...
                    if T.count('T')>0:
                        ...
                        if PARTICLE_NAMES[p1]=='photon':
                            with mpl.rc_context({'path.sketch': (5, 15, 1)}):
                                ax.plot(T[st:end],[Xi[st:end] for Xi in X][0],color=PARTICLE_DICT[PARTICLE_NAMES[p1]]['color'],linestyle=PART_LStyle[p0])
                        else:
                            ax.plot(T[st:end],[Xi[st:end] for Xi in X][0],color=PARTICLE_DICT[PARTICLE_NAMES[p1]]['color'],linestyle=PART_LStyle[p0])
                        T.remove('T')
                        for xd in range(DIM_Numb): 
                        ...
                        if PARTICLE_NAMES[p1] in ['photon','w-boson','w+boson','zboson']:
                            with mpl.rc_context({'path.sketch': (5, 15, 1)}):
                                ax.plot(T[st:],[Xi[st:] for Xi in X][0],color=PARTICLE_DICT[PARTICLE_NAMES[p1]]['color'],linestyle=PART_LStyle[p0])
                        else:
                            ax.plot(T[st:],[Xi[st:] for Xi in X][0],color=PARTICLE_DICT[PARTICLE_NAMES[p1]]['color'],linestyle=PART_LStyle[p0])
    ...
    for col in COLPTS:
        ax.plot(col[0],col[1],ms=4,marker=Event_Style,color=Event_Colors[col[2]-1])
