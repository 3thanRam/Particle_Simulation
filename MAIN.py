...
def COUNTFCT(LIST, elem,mode=None):
    ...
    if len(elem)==2:#id,type search
        ...
        if LIST.size==0:
            ...
            Testequal=np.array_equal(LIST[0],elem)#=LIST[0]==elem #Testequal=np.equal(LIST[0],elem)
...
def GetCoefs(F,t):
    ...
    """
    COEFLIST=[]
    Param_INTERPOS,Param_POS,Param_Velocity,Param_Time,Param_ID_TYPE,Param_endtype=F
    ...
    for coefgroup in range(len(Param_INTERPOS)):
        ...
        xinters = np.array([Interpos for Interpos in Param_INTERPOS[coefgroup]], dtype=object)
        ...
        for ind0 in range(len(id_type)):
            ...
            if ends[ind0] == 0:
                ...
                if BOUNDARY_COND == 0:
                    ...
                    for r in range(ends[ind0]):
                        b.append(xinters[ind0][r][0] - A * float(ts[ind0][1+r]))
                        flipindex,flipvalue=Global_variables.Vflipinfo[id_type[ind0][2]][id_type[ind0][1]][id_type[ind0][0]][r]
                    ...
                    b.append(xfs[ind0] - A * float(ts[ind0][0]))
            COEFS.append([a, b, ts[ind0], [id_type[ind0][1],id_type[ind0][2]],int(id_type[ind0][0]),xinters[ind0],ends[ind0]])
                ...
                for z1, b1 in enumerate(B1): #try saving as (0,b1),(1,b2)
                    ...
                    for z2, b2 in enumerate(B2):
                        ...
                        if Collisiontype != 0 :
                            ...
                            if INTER[1]>mintime:
                                continue
                            colinfo=[INTER[1],INTER[2],Collisiontype,z1,z2,p1,id1,p2,id2]
                            searchinfo=[INTER[1],INTER[2],p1,id1,p2,id2]
                            
                            Clptinfo=[INTER[1],INTER[2],Collisiontype]
                            if COUNTFCT(INTERACT_SEARCH,searchinfo)==0 and COUNTFCT(Global_variables.COLPTS,Clptinfo)==0:
                                NumbCols+=1
...
def Gen_Field(Xarray,SystemList):
    ...
    loc_arr=np.array([[Xiarray['Pos'][i] for Xiarray in Xarray] for i in range(len(Xarray[0]))])
    charges=((-1)**(Xarray[0]['TypeID0']))*np.array([PARTICLE_DICT[PARTICLE_NAMES[index]]['charge'] for index in Xarray[0]['TypeID1']])
    ...
    QUARK_SPONT_NAMES=['up_Quark','down_Quark','strange_Quark','charm_Quark','bottom_Quark','top_Quark']
    EcreateLIST=[2*PARTICLE_DICT[partName]['mass']*C_speed**2 for partName in QUARK_SPONT_NAMES]
...
def main(T,n1,n2,vo,l,Numb_Dimensions,BoundsCond,D,File_path_name=None):
    ...
    LO,LOinf=np.array([l[d] for d in range(Numb_Dimensions)]),np.array([0 for d in range(Numb_Dimensions)])
    ...
    for ti in range(1,T):
        ...
        Global_variables.Vflipinfo=[[[[] for ni in range(maxnumbpart+1)],[[] for ni in range(maxnumbanti+1)]] for maxnumbpart,maxnumbanti in Global_variables.MaxIDperPtype]
        ...
        Xi=np.array([[(s.X[d],s.parity[0],s.parity[1],s.ID) for s in SYST] for d in range(DIM_Numb)], dtype=dtype)
        
        
        Global_variables.FIELD=Gen_Field(Xi,SYST) #update electric field according to positions and charges of particles
        ...
        Xf=np.array([[(DOINFOLIST[s][0][d],DOINFOLIST[s][1][0],DOINFOLIST[s][1][1],DOINFOLIST[s][2]) for s in range(len(SYST))]for d in range(DIM_Numb)], dtype=dtype) 
        Xf.sort(order='Pos')
        ...
        INI_TYPE_PARTorANTI=[np.array([elem[1] for elem in Xi[d]]) for d in range(DIM_Numb)]
        INI_TYPE_CHARGE=[np.array([elem[2] for elem in Xi[d]]) for d in range(DIM_Numb)]

        END_TYPE_PARTorANTI=[np.array([elem[1] for elem in Xf[d]]) for d in range(DIM_Numb)]
        END_TYPE_CHARGE=[np.array([elem[2] for elem in Xf[d]]) for d in range(DIM_Numb)]


        CHANGESdim=[np.where((Xi[d]['index']!=Xf[d]['index']) | (INI_TYPE_PARTorANTI[d]!=END_TYPE_PARTorANTI[d])| (INI_TYPE_CHARGE[d]!=END_TYPE_CHARGE[d])) for d in range(DIM_Numb)]
        ...
        if DIM_Numb==1:
            ...
            CHANGES=np.intersect1d(np.intersect1d(CHANGESdim[0],CHANGESdim[1]),CHANGESdim[2])
        ...
        #TYPE_ARRAY=np.array()
        for chg in CHANGES:#  particles of index between ini and end could interact with the particle
            CHGind.append([])
            ...
            for d in range(DIM_Numb):
                matchval=np.where((Xi[d]['index'][chg]==Xf[d]['index']) & (INI_TYPE_PARTorANTI[d][chg]==END_TYPE_PARTorANTI[d])& (INI_TYPE_CHARGE[d][chg]==END_TYPE_CHARGE[d]))[0][0]
                ...
                if abs(Xa-Xb)<=distmax[d]:
                    ...
                    Inflist=np.where((Xf[d]['Pos']<=Xinf)&(Xf[d]['Pos']>=Xsup-distmax[d]))[0]
                    Suplist=np.where((Xf[d]['Pos']>=Xsup)&(Xf[d]['Pos']<=Xinf+distmax[d]))[0]
                    if Suplist.size==0:
                        maxi=max(chg,matchval)+1
                    else:
                ...
                for elem_ind in range(mini,maxi):
                    doindex=np.where((DO_INDEX==Xf[d]['index'][elem_ind])&(DO_TYPE_PARTorANTI==Xf[d]['TypeID0'][elem_ind])&(DO_TYPE_CHARGE==Xf[d]['TypeID1'][elem_ind]))[0][0]
                    ...
                    POS,TYPE0,TYPE1,ID=Xf[d]['Pos'][elem_ind],Xf[d]['TypeID0'][elem_ind],Xf[d]['TypeID1'][elem_ind],Xf[d]['index'][elem_ind]
                    ...
                    for d2 in range(1,DIM_Numb):
                        ...
                        if d2>=DIM_Numb:
                            d2-=DIM_Numb
                        posind2=np.where((Xf[d2]['index']==ID) & (Xf[d2]['TypeID0']==TYPE0)& (Xf[d2]['TypeID1']==TYPE1))[0][0]
                        #posind2=np.where((Xf[d2]['index']==ID) & (Xf[d2]['Type']==TYPE))[0][0]
                        POSLIST[d2]=Xf[d2]['Pos'][posind2]
                    if  CHGind[-1]==[] or COUNTFCT(Param_ID_TYPE[-1],[ID,TYPE0,TYPE1],1)==0 :#
        ...
        if BOUNDARY_COND==1:
            BOUNDARYCHECKS=[np.where((Xi[:]['Pos']<(Linf[:,np.newaxis]+Vmax[:,np.newaxis]*dt)).any(axis=0))[0],np.where((Xi[:]['Pos']>(L[:,np.newaxis]-Vmax[:,np.newaxis]*dt)).any(axis=0))[0]]
        else:
            BOUNDARYCHECKS=[np.where((Xi['Pos']<(Linf[:,np.newaxis]+Vmax[:,np.newaxis]*dt)).any(axis=0)|(Xi['Pos']>(L[:,np.newaxis]-Vmax[:,np.newaxis]*dt)).any(axis=0))[0]]
        ...
        for Bcheck in BOUNDARYCHECKS:
            if len(Bcheck)>0:
                ...
                for PART_B_inf_index in Bcheck:
                    for d in range(DIM_Numb):
                        elem_ind=np.where((Xf[d]['index']==Xi[d]['index'][PART_B_inf_index]) & (Xi[d]['TypeID0'][PART_B_inf_index]==END_TYPE_PARTorANTI[d])& (Xi[d]['TypeID1'][PART_B_inf_index]==END_TYPE_CHARGE[d]))[0][0]
                        doindex=np.where((DO_INDEX==Xf[d]['index'][elem_ind])&(DO_TYPE_PARTorANTI==Xf[d]['TypeID0'][elem_ind])&(DO_TYPE_CHARGE==Xf[d]['TypeID1'][elem_ind]))[0][0]
                        InterPos,Velocity,TimeParams,Endtype=get2(DOINFOLIST[doindex])
                        POS,TYPE0,TYPE1,ID=Xf[d]['Pos'][elem_ind],Xf[d]['TypeID0'][elem_ind],Xf[d]['TypeID1'][elem_ind],Xf[d]['index'][elem_ind]
                        ...
                        for d2 in range(1,DIM_Numb):
                            ...
                            posind2=np.where((Xf[d2]['index']==ID) & (END_TYPE_PARTorANTI[d2]==TYPE0)& (END_TYPE_CHARGE[d2]==TYPE1))[0][0]
                            POSLIST[d2]=Xf[d2]['Pos'][posind2]
                        CHGind[-1].append(elem_ind)
                        Param_INTERPOS[-1].append(InterPos)
                        Param_POS[-1].append(POSLIST)
        ...
        PARAMS=[Param_INTERPOS,Param_POS,Param_Velocity,Param_Time,Param_ID_TYPE,Param_endtype]#[Param_INTERPOS,Param_POS,Param_Velocity,Param_Time,Param_ID_TYPE,Param_endtype]
        ...
        for I1,I2 in combinations(range(len(Param_ID_TYPE)),2):  
            ...
            nrows, ncols = A.shape
            dtypeO={'names':['f{}'.format(i) for i in range(ncols)],'formats':ncols * [A.dtype]}

            Overlap,inda,indb = np.intersect1d(A.view(dtypeO), B.view(dtypeO),return_indices=True)
        ...
        REMOVELIST.sort(reverse=True)#removing top to bottom to avoid index changes after each removal
        ...
        for indUpdate in range(len(Xf[0])): #dimension 0
            ...
            for d in range(1,DIM_Numb):
                Pos_d_index=np.where((Xf[d]['index']==id_search) & (Xf[d]['TypeID0']==type_search[0]) & (Xf[d]['TypeID1']==type_search[1]))[0][0]
            ...
            for s in Global_variables.SYSTEM[Conv_search_Index][type_search[0]]:
                if s.ID==id_search :
                    ...
                    s.X=pos_search
                    doindex=np.where((DO_INDEX==id_search)&(DO_TYPE_PARTorANTI==type_search[0])&(DO_TYPE_CHARGE==type_search[1]))[0][0]
                    ...
                    if Endtype>0:
                        for nz in range(len(targs)-1):
                            Global_variables.TRACKING[Conv_search_Index][partORanti][id_search].extend([[targs[nz+1],xinterargs[nz][0]],['T','X'],[targs[nz+1],xinterargs[nz][1]]])
                        Global_variables.ALL_TIME.extend(targs[1:])
                    Global_variables.TRACKING[Conv_search_Index][partORanti][id_search].append([t,pos_search])
                    break
    ...
    if D==1: #if D is equal to 1, then the function produces a graphical output of the simulation results using the DRAW function.
        ...
        DRAW_PARAMS=[T,dt,[Linf,L],DIM_Numb,Global_variables.COLPTS,Global_variables.TRACKING,Dens,L_FCT,BOUNDARY_COND,Global_variables.ALL_TIME,PARTICLE_DICT]
        if File_path_name!=None:
            DRAW_PARAMS.append(File_path_name)
        DRAW_TRAJ.DRAW(*DRAW_PARAMS)
    elif D==0: #the function produces a density plot of the simulation results using the DENS_FCT function
